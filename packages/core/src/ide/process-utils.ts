 ...
 * process).
 *
 * This function can be overridden by setting the `GEMMA_CLI_IDE_PID`
 * environment variable. This is useful for launching Gemma CLI in a
 * standalone terminal while still connecting to an IDE instance.
 *
 * If `GEMMA_CLI_IDE_PID` is set, the function uses that PID and fetches
 * the command for it.
 *
...
export async function getIdeProcessInfo(): Promise<{
  ...
  if (process.env['GEMMA_CLI_IDE_PID']) {
    const idePid = parseInt(process.env['GEMMA_CLI_IDE_PID'], 10);
