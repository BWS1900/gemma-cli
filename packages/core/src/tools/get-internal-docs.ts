...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find Gemma CLI documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available Gemma CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to Gemma CLI's internal documentation.
