...
describe('sanitizeEnvironment', () => {
  ...
  it('should allow variables prefixed with GEMMA_CLI_', () => {
    const env = {
      GEMMA_CLI_FOO: 'bar',
      GEMMA_CLI_BAZ: 'qux',
    };
  ...
  describe('value-first security: secret values must be caught even for allowed variable names', () => {
    ...
    it('should NOT redact GEMMA_CLI_ variables even if their value looks like a secret (fully trusted)', () => {
      const env = {
        GEMMA_CLI_INTERNAL: 'ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
      };
      const sanitized = sanitizeEnvironment(env, EMPTY_OPTIONS);
      expect(sanitized).toEqual(env);
    });
