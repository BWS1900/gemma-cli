...
/**
 * Service for managing Git worktrees within Gemma CLI.
...
export function getWorktreePath(projectRoot: string, name: string): string {
  return path.join(projectRoot, '.gemma', 'worktrees', name);
}
...
export function isGeminiWorktree(
  ...
  try {
    ...
    const worktreesBaseDir = path.join(realProjectRoot, '.gemma', 'worktrees');
