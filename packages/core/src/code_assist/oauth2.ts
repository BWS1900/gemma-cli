...
async function initOauthClient(
  ...
  if (config.isBrowserLaunchSuppressed()) {
    if (!config.isInteractive()) {
      throw new FatalAuthenticationError(
        'Manual authorization is required but the current session is non-interactive. ' +
          'Please run the Gemma CLI in an interactive terminal to log in, ' +
          'provide a GEMMA_API_KEY, or ensure Application Default Credentials are configured.',
      );
    }
    let success = false;
    const maxRetries = 2;
    ...
    try {
      ...
      exitAlternateScreen();
      // If this was triggered from an active Gemma CLI TUI this event ensures
