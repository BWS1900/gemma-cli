...
import { GemmaRespondingSpinner } from './GemmaRespondingSpinner.js';
...
vi.mock('./GemmaSpinner.js', () => ({
  GemmaSpinner: ({ altText }: { altText?: string }) => (
    <Text>GemmaSpinner {altText}</Text>
  ),
}));

describe('GemmaRespondingSpinner', () => {
  ...
  it('renders spinner when responding', async () => {
    ...
    const { lastFrame, unmount } = await render(<GemmaRespondingSpinner />);
    expect(lastFrame()).toContain('GemmaSpinner');
    unmount();
  });
  ...
  it('renders non-responding display when provided', async () => {
    ...
    const { lastFrame, unmount } = await render(
      <GemmaRespondingSpinner nonRespondingDisplay="Waiting..." />,
    );
