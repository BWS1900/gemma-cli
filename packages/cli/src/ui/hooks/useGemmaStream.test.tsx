...
const MockedGemmaClientClass = vi.hoisted(() =>
...
vi.mock('@google/gemini-cli-core', async (importOriginal) => {
  ...
  return {
    ...
    GitService: vi.fn(),
    GemmaClient: MockedGemmaClientClass,
...
describe('useGeminiStream', () => {
  ...
  const mockGetGemmaClient = vi.fn().mockImplementation(() => {
    const clientInstance = new MockedGemmaClientClass(mockConfig);
  ...
  const mockConfig: Config = {
    ...
    getGemmaClient: mockGetGemmaClient,
  ...
  beforeEach(() => {
    ...
    // Reset mocks for GemmaClient instance methods (startChat and sendMessageStream)
    // The GemmaClient constructor itself is mocked at the module level.
    mockStartChat.mockClear().mockResolvedValue({
      sendMessageStream: mockSendMessageStream,
    } as unknown as any); // GemmaChat -> any
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.gemma/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  const renderTestHook = async (
    ...
    const client = gemmaClient || mockConfig.getGemmaClient();
  ...
  const renderHookWithDefaults = async (
    ...
    return renderHookWithProviders(() =>
      useGeminiStream(
        new MockedGemmaClientClass(mockConfig),
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const client = new MockedGemmaClientClass(mockConfig);
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should record client-initiated tool calls in GemmaChat history', async () => {
      const { result, client: mockGemmaClient } = await renderTestHook();
      ...
      // Verify that the tool call and response were added to GemmaChat history
      expect(mockGemmaClient.addHistory).toHaveBeenCalledWith({
        role: 'model',
        parts: [
          {
            functionCall: {
    ...
    it('should NOT record other client-initiated tool calls in history', async () => {
      ...
      expect(mockGemmaClient.addHistory).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Error Handling', () => {
    it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
      ...
      const { result } = await renderHookWithProviders(() =>
        useGeminiStream(
          new MockedGemmaClientClass(testConfig),
          [],
          mockAddItem,
          testConfig,
          mockLoadedSettings,
  ...
  describe('Loop Detection Confirmation', () => {
    beforeEach(() => {
      ...
      mockConfig.getGemmaClient = vi.fn().mockReturnValue({
        ...new MockedGemmaClientClass(mockConfig),
    ...
    it('should disable loop detection and show message when user selects "disable"', async () => {
      ...
      mockConfig.getGemmaClient = vi.fn().mockReturnValue(mockClient);
