...
export const useGeminiStream = (
  ...
  const [
    ...
    async (completedToolCallsFromScheduler) => {
      ...
      if (completedToolCallsFromScheduler.length > 0) {
        ...
        try {
          const currentModel =
            config.getGemmaClient().getCurrentSequenceModel() ??
            config.getModel();
          config
            .getGemmaClient()
            .getChat()
  ...
  const handleContentEvent = useCallback(
    (
      ...
      currentGemmaMessageBuffer: string,
      userMessageTimestamp: number,
    ): string => {
      setRetryStatus(null);
      if (turnCancelledRef.current) {
      ...
      let newGemmaMessageBuffer = currentGemmaMessageBuffer + eventValue;
      if (
        ...
        newGemmaMessageBuffer = eventValue;
      }
      // Split large messages for better rendering performance. Ideally,
      // we should maximize the amount of output sent to <Static />.
      const splitPoint = findLastSafeSplitPoint(newGemmaMessageBuffer);
      if (splitPoint === newGemmaMessageBuffer.length) {
        // Update the existing message with accumulated content
        setPendingHistoryItem((item) => ({
          // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
          type: item?.type as 'gemini' | 'gemini_content',
          text: newGemmaMessageBuffer,
        }));
      } else {
        ...
        const beforeText = newGemmaMessageBuffer.substring(0, splitPoint);
        const afterText = newGemmaMessageBuffer.substring(splitPoint);
        ...
        newGemmaMessageBuffer = afterText;
      }
      return newGemmaMessageBuffer;
    },
  ...
  const handleChatCompressionEvent = useCallback(
    (
      eventValue: ServerGemmaChatCompressedEvent['value'],
      userMessageTimestamp: number,
    ) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
      ...
      runInDevTraceSpan(
        ...
        async ({ metadata: spanMetadata }) => {
          ...
          return promptIdContext.run(prompt_id, async () => {
            ...
            try {
              const stream = gemmaClient.sendMessageStream(
                queryToSend,
                abortSignal,
                prompt_id!,
                undefined,
  ...
  const handleApprovalModeChange = useCallback(
    ...
    [config, toolCalls, gemmaClient, streamingState, addItem, onDebugMessage],
  );
  ...
  }, [toolCalls, config, onDebugMessage, gitService, history, gemmaClient]);
