...
import { TestRig, GEMMA_DIR } from './test-helper.js';
...
describe('User Policy Regression Repro', () => {
  ...
  it('should respect policies in ~/.gemma/policies/allowed-tools.toml', async () => {
    rig.setup('user-policy-test', {
      fakeResponsesPath: join(import.meta.dirname, 'user-policy.responses'),
    });

    // Create ~/.gemma/policies/allowed-tools.toml
    const userPoliciesDir = join(rig.homeDir!, GEMMA_DIR, 'policies');
