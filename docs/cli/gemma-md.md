# Provide context with GEMMA.md files

Context files, which use the default name `GEMMA.md`, are a powerful feature
for providing instructional context to the Gemma model. You can use these files
...
1.  **Global context file:**
    - **Location:** `~/.gemma/GEMMA.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Environment and workspace context files:**
    - **Location:** The CLI searches for `GEMMA.md` files in your configured
...
3.  **Just-in-time (JIT) context files:**
    - **Location:** When a tool accesses a file or directory, the CLI
      automatically scans for `GEMMA.md` files in that directory and its
...
### Example `GEMMA.md` file

Here is an example of what you can include in a `GEMMA.md` file at the root of
...
- **`/memory reload`**: Forces a re-scan and reload of all `GEMMA.md` files
  from all configured locations.

## Modularize context with imports

You can break down large `GEMMA.md` files into smaller, more manageable
...
While `GEMMA.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "GEMMA.md"]
  }
}
```
...
- Learn about [Ignoring files](./gemma-ignore.md) to exclude content from the
  context system.
